  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response && response.idprogetto) {
        ...
        dispatch({ type: 'set', projects: newData.values })
        dispatch({ type: 'set', id: { id: idprogetto } })
        history(`/dashboard/projectitems/${idprogetto}`, { state: { id: idprogetto } })
      }
    } catch (error) {
      console.log('Error in newProject component while submitting data: ' + error)
      alert(
              ...
              <FormControl fullWidth>
                ...
                >
                  {info.pms.split(';').map((pm, index) => (
                    <MenuItem key={index} value={pm}>{pm}</MenuItem>
                  ))}
                </Select>
              </FormControl>
              <TextField
