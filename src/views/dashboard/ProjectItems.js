...
const ProjectItems = () => {
  ...
  const handleInputChange = (e) => {
    ...
    // Aggiorna le richieste pendenti
    let newRequests = [...pendingRequests];
    if (name === 'projectName') {
      newRequests = newRequests.filter(req => !req.startsWith('Nome progetto aggiornato:'));
      newRequests.push(`Nome progetto aggiornato: ${value}`);
  ...
  return (
    ...
    >
      ...
      <Box sx={{ width: '99%', mt: '2rem' }}>
        <AppTable ref={ref} columns={columnDefs} rows={projectItems.values} showActions={true} disableCheckboxSelection={true} onRowDoubleClick={() => {}} />
      </Box>
